#ifndef __RAQSAC_RaqsacSocket_H
#define __RAQSAC_RaqsacSocket_H
...
#include "../raqsac/RaqsacCommand_m.h"

namespace inet {

class RaqsacStatusInfo;

class INET_API RaqsacSocket : public ISocket
{
public:
    /**
    ...
    public:
         ...
         */
        virtual void socketDataArrived(RaqsacSocket *socket, Packet *packet, bool urgent) = 0;
        virtual void socketAvailable(RaqsacSocket *socket, RaqsacAvailableInfo *availableInfo) = 0;
        virtual void socketEstablished(RaqsacSocket *socket) = 0;
        virtual void socketPeerClosed(RaqsacSocket *socket) = 0;
        virtual void socketClosed(RaqsacSocket *socket) = 0;
        virtual void socketFailure(RaqsacSocket *socket, int code) = 0;
        virtual void socketStatusArrived(RaqsacSocket *socket, RaqsacStatusInfo *status) = 0;
        virtual void socketDeleted(RaqsacSocket *socket) = 0;
    };
...
protected:
    ...
    cGate *gateToRaqsac = nullptr;
    std::string raqsacAlgorithmClass;

protected:
    void sendToRaqsac(cMessage *msg, int c = -1);
...
public:
     ...
     */
    static const char* stateName(RaqsacSocket::State state);

    void setState(RaqsacSocket::State state)
    {
        sockstate = state;
    }
    ;
     ...
     */
    void setOutputGate(cGate *toRaqsac)
    {
        gateToRaqsac = toRaqsac;
    }

    /**
     * Binds the provided local address and port to the RaqsacSocket.
     */
    void bind(L3Address localAddr, int localPort);

    /**
     * Returns the current raqsacAlgorithmClass parameter.
     */
    const char* getRaqsacAlgorithmClass() const
    {
        return raqsacAlgorithmClass.c_str();
    }

    /**
     * Sets the ndpAlgorithmClass parameter of the next connect() or listen() call.
     */
    void setRaqsacAlgorithmClass(const char *raqsacAlgorithmClass)
    {
        this->raqsacAlgorithmClass = raqsacAlgorithmClass;
    }
     ...
     * get a new connId (and thus, must be handled with a new RaqsacSocket object),
     * while the original connection (original connId) will keep listening on
     * the port. The new RaqsacSocket object must be created with the
     * RaqsacSocket(cMessage *msg) constructor.
     *
     * If you need to handle multiple incoming connections, the RaqsacSocketMap
     ...
     * numSymbolsToSend needed to fill sendQueue with number of packets to send to the receiver.
     */
    void connect(L3Address localAddress, L3Address remoteAddr, int remotePort, unsigned int numSymbolsToSend, unsigned int priorityValue);
     ...
     * established RaqsacSocket object. The renewSocket is used to ensure that the socket has not been
     ...
     * has a RaqsacCommand as getControlInfo(), and the connId in it matches
     * that of the socket.)
     */
     ...
     * declared it as
     * <pre>
     * class MyAppModule : public cSimpleModule, public RaqsacSocket::CallbackInterface
     * </pre>
     * and redefined the necessary virtual functions; or you may use
     * dedicated class (and objects) for this purpose.
     *
     * RaqsacSocket doesn't delete the callback object in the destructor
     * or on any other occasion.
     *
     * YourPtr is an optional pointer. It may contain any value you wish --
     * RaqsacSocket will not look at it or do anything with it except passing
...
#endif // ifndef __INET_RaqsacSocket_H
