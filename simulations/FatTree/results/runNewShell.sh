...
opp_run_release -u Cmdenv -m -n ../..:../../../src:../../../../inet4/src:../../../../inet4/examples:../../../../inet4/tutorials:../../../../inet4/showcases -l ../../../src/scdp -l ../../../../inet4/src/INET ../exp1-1-seed1.ini -c General -r "\$FatTreeSize==$(printf '%d' $1)  &&  \$numShortFlows==$(printf '%d' $2)"

echo "Throughput"
scavetool export -T s -f "module(**.app[*]) AND ("mohThroughputRQ:mean")"   -F CSV-S -o instThroughput.csv *.sca
echo "   "

echo "FCT"
scavetool export -T s -f "module(**.app[*]) AND ("fctRecordv2:last")"   -F CSV-S -o fct.csv *.sca
echo "   "
...
#echo "tcp RTO"
#scavetool export -T v -f 'module(**.raptorQ) AND ("numRTOsMOH:vector" )'  -F CSV-R -o numRTOs.csv *.vec
#echo "   "
#
#echo "tcp FastRetransmissions"
#scavetool export -T v -f 'module(**.raptorQ) AND ( "numFastRetransmissions:vector" )'  -F CSV-R -o numFastRets.csv  *.vec
#echo "   "
#
#
#
...
scavetool export -T s -f 'module(FatTree.Pod[*].racks[*].servers[*].ppp[*].queue) AND  name("rcvdPk:sum(packetBytes)") '   -F CSV-S -o serversRcvdPktBytes.csv *.sca

echo "servers-dropPk:sum(packetBytes)"
scavetool export -T s -f 'module(FatTree.Pod[*].racks[*].servers[*].ppp[*].queue) AND  name("dropPk:sum(packetBytes)") '   -F CSV-S -o serversDropPktBytes.csv *.sca
echo "servers-rcvdPk:count"
scavetool export -T s -f 'module(FatTree.Pod[*].racks[*].servers[*].ppp[*].queue) AND  name("rcvdPk:count") '   -F CSV-S -o serversRcvdPkt.csv *.sca
echo "servers-dropPk:count"
scavetool export -T s -f 'module(FatTree.Pod[*].racks[*].servers[*].ppp[*].queue) AND  name("dropPk:count") '   -F CSV-S -o serversDropPkt.csv *.sca
#
echo "   "

echo "config."
scavetool export -T s -f " name(simTimeTotal=)  OR name(numShortFlows=)  OR name(flowSize=) OR name(percentLongFlowNodes=) OR name(arrivalRate=) OR name(randTM)  OR name(permTM)  OR name(wallClockTime=) OR name(IW=) OR name(ndpSwitchQueueLength=) OR name(perPacketEcmp=) OR name(perFlowEcmp=) OR name(oneToOne=) OR name(oneToMany=) OR name(manyToOne=)  OR name(seedValue=) OR name(kValue=)"   -F CSV-S -o config.csv *.sca
