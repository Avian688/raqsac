...
opp_run_release  -u Cmdenv -m -n ../../../..:../../../../../examples:../../../../../tutorials -l ../../../../INET ../with.ini -c General -r "\$FatTreeSize==$(printf '%d' $1)  &&  \$numShortFlows==$(printf '%d' $2)"

echo "Throughput"
scavetool export -T s -f "module(**.raptorQApp[*]) AND ("mohThroughputRQ:mean")  "   -F CSV-S -o instThroughput.csv *.sca
echo "   "
echo "FCT"
...
scavetool export -T s -f "module(**.raptorQApp[*]) AND name(totalBytesRcvdSink=) "   -F CSV-S -o totalbytesrcvdsink.csv *.sca


echo "   "
echo "header"
scavetool export -T s -f "module(**.raptorQApp[*]) AND ("numRcvTrimmedHeaderSig:last")"   -F CSV-S -o numRcvHeader.csv *.sca
echo "   "
